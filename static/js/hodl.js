...
function createResultsTable(data) {
    $('div.returns').each(function (index) {
        ...
        table_2 = '<table class="table table-striped" style="width: 100%">';
        table_2 += '<thead>';
        table_2 += '<tbody>';
        table_2 += '<tr>';
        table_2 += '<td class="text-start">Starting Value</td>';
        table_2 += `<td class="text-end">${formatNumber(data['ticker_start_value'], 0)}</td>`;
        table_2 += '</tr>';
        table_2 += '<tr>';
        table_2 += '<td class="text-start">Final Value</td>';
        table_2 += `<td class="text-end">${formatNumber(data['ticker_end_value'], 0)}</td>`;
        table_2 += '</tr>';
        table_2 += '<tr>';
        table_2 += '<td class="text-start">Total Return in Period</td>';
        table_2 += `<td class="text-end">${formatNumber(data['period_tr'] * 100, 2, '', '%')}</td>`;
        table_2 += '</tr>';
        table_2 += '<tr>';
        table_2 += '<td class="text-start">Period excluding largest</td>';
        table_2 += `<td class="text-end">${formatNumber(data['exclude_nlargest_tr'] * 100, 2, '', '%')}</td>`;
        table_2 += '</tr>';
        table_2 += '<tr>';
        table_2 += '<td class="text-start">Period excluding smallest</td>';
