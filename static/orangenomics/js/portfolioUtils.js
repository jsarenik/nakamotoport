$(document).ready(function () {
    ...
    $(document).on('click', '#go_back', function () {
        p = encodeURIComponent(JSON.stringify(data.METADATA.portfolio))
        s = encodeURIComponent(JSON.stringify(data.METADATA.start_date))
        e = encodeURIComponent(JSON.stringify(data.METADATA.end_date))
        r = encodeURIComponent(JSON.stringify(data.METADATA.rebalance))
        a = encodeURIComponent(JSON.stringify(data.METADATA.allocations))

        $('#loading_modal').modal('show');
        url = base_url() + 'orangenomics/portfolio?portfolio=' + p + '&start_date=' + s + '&end_date=' + e + '&rebalance=' + r + '&allocations=' + a
    ...
    $(document).on('click', '#button_load_portfolio', function () {
        ...
        $('#loadModal').modal('hide');
        $('#loading_modal').modal('show');
        $('#load_message').html("Loading Portfolio...<br><span class='text-60-small'>(this can take some time depending on number of assets)</span><br><br>Please Wait...")
        // setTimeout(function () {
        //     load_portfolio();
